...
//    private By checkTimeDelay = By.xpath("//input[@id='timeDelay']/following-sibling::div[1]");
    ...
    public String getTestResult() {
        return find(testResult).waitUntilVisible().getText();
    }

    // Click the "Account" button and reset the account data
    public DashboardPage clickAccountButton() {
        find(accountButton).waitUntilVisible().click();
        return new DashboardPage();
    }

    public DashboardPage fillPasswordField() {
        find(passwordField).waitUntilVisible().sendKeys("20exitget17");
        return new DashboardPage();
    }

    public DashboardPage clickResetButton() {
        find(resetAccountButton).click();
        return new DashboardPage();
    }
//    public DashboardPage closeAlertWindow() {
